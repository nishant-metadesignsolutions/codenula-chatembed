...
const parseChatflows = () => {
  try {
    ...
    if (chatflowVars.length === 0) {
      console.error(
        "No chatflow configurations found in environment variables"
      );
      process.exit(1);
    }
    ...
    for (const [identifier, value] of chatflowVars) {
      ...
      if (domains.includes("*")) {
        console.error(
          `\x1b[31mError: Wildcard (*) domains are not allowed in ${identifier}. This flow will not be accessible.\x1b[0m`
        );
        continue;
      }
...
chatflows.forEach((config, identifier) => {
  if (isValidChatflowConfig(config.chatflowId)) {
    console.info(
      "\x1b[36m%s\x1b[0m",
      `  ${identifier}: ${config.chatflowId} (${config.domains.join(", ")})`
    );
  }
});
...
app.get("/web.js", (req, res) => {
  ...
  // const allAllowedDomains = Array.from(chatflows.values()).flatMap(
...
const handleProxy = async (req, res, targetPath) => {
  try {
    let identifier =
      req.query.chatflowId?.split("/")[0] || req.path.split("/").pop() || null;
    ...
    if (req.query.chatId && req.query.fileName) {
      ...
      if (!response.ok) {
        console.error(
          `File proxy error: ${response.status} ${response.statusText}`
        );
        return res
          .status(response.status)
          .json({ error: `File proxy error: ${response.statusText}` });
      }
    ...
    if (!response.ok) {
      ...
      return res
        .status(response.status)
        .json({ error: `Proxy error: ${response.statusText}` });
    }
...
app.post(
  "/api/v1/attachments/:identifier/:chatId",
  upload.array("files"),
  async (req, res) => {
    try {
